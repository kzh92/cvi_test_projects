...
#define  D(...)    printf(camera,__VA_ARGS__)
...
static int
_get_bayer_min_cross(const BayerDesc* desc,
const void* buf,
int x,
int y,
int width,
int height)
{
	if (x > 0 && x < (width - 1) && y > 0 && y < (height - 1))
	{
		...
		* special case it for performance reasons. */
		...
		int nHoriGrad = _get_bayer_color(desc, buf, x - 1, y, width) - _get_bayer_color(desc, buf, x + 1, y, width);
		int nVertGrad = _get_bayer_color(desc, buf, x, y - 1, width) - _get_bayer_color(desc, buf, x, y + 1, width);
...
static int
_get_bayer_min_diag(const BayerDesc* desc,
const void* buf,
int x,
int y,
int width,
int height)
{
	if (x > 0 && x < (width - 1) && y > 0 && y < (height - 1))
	{
		int nGrad1 = _get_bayer_color(desc, buf, x - 1, y - 1, width) - _get_bayer_color(desc, buf, x + 1, y + 1, width);
		int nGrad2 = _get_bayer_color(desc, buf, x + 1, y - 1, width) - _get_bayer_color(desc, buf, x - 1, y + 1, width);
		...
		if (nGrad1 > nGrad2)
		{
			return (_get_bayer_color(desc, buf, x + 1, y - 1, width) +
				_get_bayer_color(desc, buf, x - 1, y + 1, width)) / 2;
		}
		else
		{
			return (_get_bayer_color(desc, buf, x - 1, y - 1, width) +
...
int* blue)
{
	...
	if (pixel_color == 'G') {
		...
		*green = _get_bayer_color(desc, buf, x, y, width);
		...
		//*green = ((int)(_get_bayer_color(desc, buf, x, y, width) * 4) + (int)(_get_bayer_ave_diag(desc, buf, x, y, width, height) * 4)) / 8;
	...
	else if (pixel_color == 'R') 
	{
		...
		*red = _get_bayer_color(desc, buf, x, y, width);
		...
		const char next_pixel_color = _get_bayer_color_sel(desc, x + 1, y);
		const char top_pixel_color = _get_bayer_color_sel(desc, x, y + 1);
		if (next_pixel_color == 'G' && top_pixel_color == 'G')
		{
			*green = _get_bayer_ave_cross(desc, buf, x, y, width, height);
		}
		if (next_pixel_color == 'G' && top_pixel_color != 'G')
		{
			*green = _get_bayer_ave_hor(desc, buf, x, y, width, height);
		}
		if (next_pixel_color != 'G' && top_pixel_color == 'G')
		{
			*green = _get_bayer_ave_vert(desc, buf, x, y, width, height);
		}
	...
	{
		const char next_pixel_color = _get_bayer_color_sel(desc, x + 1, y);
		*green = _get_bayer_ave_diag(desc, buf, x, y, width, height);
		if (next_pixel_color == 'R')
		{
			*red = _get_bayer_ave_hor(desc, buf, x, y, width, height);
			*blue = _get_bayer_ave_vert(desc, buf, x, y, width, height);
...
static void
_get_bayerRGB_min(const BayerDesc* desc,
const void* buf,
int x,
int y,
int width,
...
int* blue)
{
	...
	if (pixel_color == 'G') {
		...
		*green = _get_bayer_color(desc, buf, x, y, width);
		...
		/* This is a red pixel. */
		*red = _get_bayer_color(desc, buf, x, y, width);
		*green = _get_bayer_min_cross(desc, buf, x, y, width, height);
		*blue = _get_bayer_min_diag(desc, buf, x, y, width, height);
	}
	else {
		/* This is a blue pixel. */
		*blue = _get_bayer_color(desc, buf, x, y, width);
		*green = _get_bayer_min_cross(desc, buf, x, y, width, height);
		*red = _get_bayer_min_diag(desc, buf, x, y, width, height);
	}
}
...
// void BAYERToYUV(const BayerDesc* bayer_fmt,
// 			const YUVDesc* yuv_fmt,
// 			const void* bayer,
// 			void* yuv,
// 			int width,
...
// 			pY += Y_next_pair, pU += UV_inc, pV += UV_inc) {
// 			int r, g, b;
// 			//_get_bayerRGB_min(bayer_fmt, bayer, x, y, width, height, &r, &g, &b);
// 			_get_bayerRGB(bayer_fmt, bayer, x, y, width, height, &r, &g, &b);
// 			//_change_white_balance_RGB(&r, &g, &b, r_scale, g_scale, b_scale);
// 			//_change_exposure_RGB_i(&r, &g, &b, exp_comp);
// 			//R8G8B8ToYUV(r, g, b, pY, pU, pV);
...
/* Generic converter from a BAYER format to a YUV format. */
void BAYERToYUV_min(const BayerDesc* bayer_fmt,
	const YUVDesc* yuv_fmt,
	const void* bayer,
	void* yuv,
	int width,
	...
	int* pnRect)
{
	...
	int nRectLeft, nRectTop, nRectHeight;
	if (pnRect)
	{
		nRectLeft = pnRect[0];
		nRectTop = pnRect[1];
		// nRectWidth = pnRect[2];
		nRectHeight = pnRect[3];
	}
	else
	{
		nRectLeft = 0;
	...
	for (y = nRectTop; y < nRectTop + nRectHeight; y++)
	{
		// uint8_t* pU = (uint8_t*)yuv + yuv_fmt->u_offset(yuv_fmt, y, width, height);
		// uint8_t* pV = (uint8_t*)yuv + yuv_fmt->v_offset(yuv_fmt, y, width, height);
		uint8_t* pY = (uint8_t*)yuv + y * width + nRectLeft;

		for (x = nRectLeft; x < nRectLeft + nRectHeight; x ++) 
		{
			int r, g, b;
			_get_bayerRGB_min(bayer_fmt, bayer, x, y, width, height, &r, &g, &b);
			//_change_white_balance_RGB(&r, &g, &b, r_scale, g_scale, b_scale);
...
{
	_load_RGB24, //.load_rgb = 
	_save_RGB24, //.save_rgb = 
	3 //.rgb_inc = 
};
...
//	1, //.U_offset = 
//	0, //.V_offset = 
//	&_UOffSepYUV, //.u_offset = 
//	&_VOffSepYUV //.v_offset = 
//};
...
static const YUVDesc _NV12 =
{
	...
	0, //.U_offset = 
	1, //.V_offset = 
	&_UOffIntrlUV, //.u_offset = 
	&_VOffIntrlUV //.v_offset = 
};
...
float exp_comp)
{
// 	BAYERToYUV(&_RG8, &_NV12,
// 			bayer, yuv, width, height,
// 			r_scale, g_scale, b_scale, exp_comp);

	BAYERToYUV(&_RG12, &_NV21,
		bayer, yuv, width, height,
		r_scale, g_scale, b_scale, exp_comp);
...
{
	BayerDesc _pRG8;
	memcpy(&_pRG8, &_RG8, sizeof(BayerDesc));
	if (szBAYER_ARRAY)
	{
		_pRG8.color_order = szBAYER_ARRAY;
	}
	BAYERToYUV_min(&_pRG8, &_NV21,
		bayer, yuv, width, height, pnRect);
}
...
float exp_comp)
{
	BAYERToRGB(&_RG8, &_RGB24,
		bayer, rgb, width, height,
		r_scale, g_scale, b_scale, 0);
}
	...
	char* szBAYER_ARRAY)
{
	...
	_get_bayerRGB_min(&_pRG8, bayer, nX, nY, width, height, &r, &g, &b);
	//_change_white_balance_RGB(&r, &g, &b, r_scale, g_scale, b_scale);
	//_change_exposure_RGB_i(&r, &g, &b, exp_comp);
	//R8G8B8ToYUV(r, g, b, pY, pU, pV);
	int nYValue = RGB2Y(r, g, b);
	return (unsigned char)nYValue;
}
