...
static void Utf16_To_WChar(UInt16* pSrc, wchar_t* pDst)
{
    int i = 0;
    while(pSrc[i])
    {
        pDst[i] = (wchar_t)(pSrc[i]);
        i ++;
    }
}

static void WChar_To_Utf16(wchar_t* pSrc, UInt16* pDst)
{
    int i = 0;
    while(pSrc[i])
    {
...
//  mbstowcs(w_dst, c_dst, 256);
//  WChar_To_Utf16(w_dst, u_dst);
...
//  PrintString(u_dst);
  ...
  FileInStream_CreateVTable(&archiveStream, NULL, 0);
...
remove(args[2]);
          ...
          memset(w_dst, 0, sizeof(w_dst));
          Utf16_To_WChar((UInt16*)destPath, w_temp);
          mbstowcs(w_dst, args[3], 256);
          wcscat(w_dst, L"/");
          wcscat(w_dst, w_temp);
          WChar_To_Utf16(w_dst, u_dst);
          printf("\n dst path:\n");
          ...
          else if (OutFile_OpenUtf16(&outFile, u_dst))
          {
          ...
          if (SzBitWithVals_Check(&db.Attribs, i))
          {
            ...
            if ((attrib & 0xF0000000) != 0)
              attrib &= 0x7FFF;
            SetFileAttributesW((LPCWSTR)w_dst, attrib);
          }
          #endif
        }
        PrintLF();
...
int extract_7z2mem(void* pInBuffer, int inLength, void** pOutBuffer, int* pOutLength)
{
	ISzAlloc allocImp;
	ISzAlloc allocTempImp;
...
//		return 1;
//	}
    archiveStream.file.file = NULL;

	FileInStream_CreateVTable(&archiveStream, pInBuffer, inLength);
	LookToRead2_CreateVTable(&lookStream, False);
	lookStream.buf = NULL;
				...
				*/
				...
				else
				{
					res = SzArEx_Extract(&db, &lookStream.vt, i,
						...
						&offset, &outSizeProcessed,
						&allocImp, &allocTempImp);
					*pOutBuffer = malloc(outBufferSize);
					*pOutLength = outBufferSize;
					memcpy(*pOutBuffer, outBuffer, outBufferSize);
					break;
					if (res != SZ_OK)
						break;
				}
