...
#include <stdio.h>
    ...
    public:
        void init(void *pBuf, uint buf_size)
        {
            m_pBuf = (uint8*)pBuf;
            m_buf_size = buf_size;
            m_buf_ofs = 0;
    ...
    {
        dct_t a = 129 - fabs((float)luma.get_px(x, y));
        dct_t b = 129 - fabs((float)luma.get_px(x + 1, y));
        return (get_px(x, y)*a
            + get_px(x + 1, y)*b) / (a + b);
    }
    ...
    inline dct_t image::blend_quad(int x, int y, image &luma)
    {
        dct_t a = 129 - fabs((float)luma.get_px(x, y));
        dct_t b = 129 - fabs((float)luma.get_px(x + 1, y));
        dct_t c = 129 - fabs((float)luma.get_px(x, y + 1));
        dct_t d = 129 - fabs((float)luma.get_px(x + 1, y + 1));
        return  (get_px(x, y)*a
            + get_px(x + 1, y)*b
            + get_px(x, y + 1)*c
...
#define JPGE_PUT_BYTE(c) { *m_pOut_buf++ = (c); if (--m_out_buf_left == 0) flush_output_buffer(); }
    ...
    bool jpeg_encoder::init(memory_stream *pStream, int width, int height, const params &comp_params)
    {
        deinit();
    ...
    bool compress_image_to_stream(memory_stream *dst_stream, int width, int height, int num_channels, const uint8 *pImage_data, const params &comp_params)
    {
        encoder.clear();
