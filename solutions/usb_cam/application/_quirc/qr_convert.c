		  ...
		  uint8_t *dst, int dst_pitch)
{
	...
	for (y = 0; y < h; y++) {
		int x;
		const uint8_t *srow = src + y * src_pitch;
        uint8_t *drow = dst + (y + 1) * dst_pitch;
...
void yuv_to_luma(const uint8_t *src, int src_pitch,
           int w, int h,
           uint8_t *dst, int dst_pitch)
{
    int y;
    ...
    for (y = 0; y < h; y++) {
        const uint8_t *yuv = src + src_pitch * (y + 1);
        uint8_t *gray = dst + y * dst_pitch;
        int i;
...
void rgb24_to_luma(const uint8_t *src, int src_pitch,
           int w, int h,
           uint8_t *dst, int dst_pitch)
{
    int y;
    ...
    for (y = 0; y < h; y++) {
        const uint8_t *rgb24 = src + src_pitch * (y + 1);
        uint8_t *gray = dst + y * dst_pitch;
        int i;
