...
static unsigned long hal_tempsen_base = 0;
...
int wait_for_finish(unsigned short chan, unsigned int timeout_ms)
{
	unsigned int time = csi_tick_get_ms();
	while (!(TEMPSEN_GET(sta_tempsen_intr_raw) & chan)) {
...
unsigned int read_temp(unsigned short chan)
{
	return TEMPSEN_GET(sta_tempsen_ch0_result);
}

unsigned int read_max_temp(unsigned short chan)
{
